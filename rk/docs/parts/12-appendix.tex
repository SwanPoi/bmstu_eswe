...
\section*{Исходный код загружаемого модуля}
\begin{lstlisting}[label=lst:all_code,caption=Загружаемый модуль ядра]
...
MODULE_DESCRIPTION("LKM for intercepting scheduler actions (Kernel 6.8.0)");
...
{
	printk(KERN_INFO "Module: Unloading module\n");
	remove_hooks();
	remove_proc_entry(FILENAME, NULL);
	free_process_sched_data_list();
